select a.figi_title                                                   figi_title,
       ...
       coalesce(round(100 * (a.last_price_pt - a.first_price_pt) / a.first_price_pt, 2), -555) profit_by_invest,
       a.offers                                                       offers,
	   coalesce(a.first_price, -555)                                  first_price,
       coalesce(a.first_price_pt, -555)                                  first_price_pt,
       coalesce(a.last_price_pt, -555)                                   last_price_pt
       , a.figi                                                       figi
...
from (select o.figi_title,
             ...
             (select c.closing_price from candle c where c.figi = o.figi
              ...
              order by c.date_time desc limit 1) last_price_pt,
	         ...
	         (select oi.purchase_price from offer oi where oi.figi = o.figi AND oi.strategy = o.strategy order by oi.id limit 1)         first_price_pt
      from offer o
	  WHERE o.purchase_date_time > '2025-08-23 00:00:00'
